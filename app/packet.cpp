...
void from_buffer (GPS& g, Buffer& b) {
	b.set_pos(266);
	char dataset_id[3];
	for (int i=0; i<2; i++) { dataset_id[i] = static_cast<char>(b.get(8)); }
	dataset_id[2] = '\0';
	std::copy(dataset_id, dataset_id+3, g.dataset_id);
	g.timestamp = static_cast<uint32_t>(b.get(32));
	g.gps_time = static_cast<uint32_t>(b.get(32));
...
void from_buffer (IMU& i, Buffer& b) {
	...
	i.timestamp = static_cast<uint32_t>(b.get(32));
	int16_t mag_x[5];
	for (int i=0; i<5; i++) { mag_x[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(mag_x, mag_x+5, i.mag_x);
	int16_t mag_y[5];
	for (int i=0; i<5; i++) { mag_y[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(mag_y, mag_y+5, i.mag_y);
	int16_t mag_z[5];
	for (int i=0; i<5; i++) { mag_z[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(mag_z, mag_z+5, i.mag_z);
	int16_t gyro_x[5];
	for (int i=0; i<5; i++) { gyro_x[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(gyro_x, gyro_x+5, i.gyro_x);
	int16_t gyro_y[5];
	for (int i=0; i<5; i++) { gyro_y[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(gyro_y, gyro_y+5, i.gyro_y);
	int16_t gyro_z[5];
	for (int i=0; i<5; i++) { gyro_z[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(gyro_z, gyro_z+5, i.gyro_z);
	int16_t accel_x[5];
	for (int i=0; i<5; i++) { accel_x[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(accel_x, accel_x+5, i.accel_x);
	int16_t accel_y[5];
	for (int i=0; i<5; i++) { accel_y[i] = static_cast<int16_t>(b.get(16)); }
	std::copy(accel_y, accel_y+5, i.accel_y);
	int16_t accel_z[5];
	for (int i=0; i<5; i++) { accel_z[i] = static_cast<int16_t>(b.get(16)); }
...
void from_buffer (Img& i, Buffer& b) {
	...
	char image_data[3];
	for (int i=0; i<2; i++) { image_data[i] = static_cast<char>(b.get(8)); }
	image_data[2] = '\0';
...
void from_buffer (Health& h, Buffer& b) {
	...
	h.timestamp = static_cast<uint32_t>(b.get(32));
	h.obc_temperature = static_cast<int8_t>(b.get(8));
	...
	h.battery_voltage = static_cast<int16_t>(b.get(10));
	h.battery_current = static_cast<int16_t>(b.get(10));
	h.battery_temperature = static_cast<int16_t>(b.get(10));
	h.charge_current = static_cast<int16_t>(b.get(10));
	h.mppt_voltage = static_cast<int16_t>(b.get(10));
	h.solar_n1_current = static_cast<int16_t>(b.get(10));
	h.solar_n2_current = static_cast<int16_t>(b.get(10));
	h.solar_e1_current = static_cast<int16_t>(b.get(10));
	h.solar_e2_current = static_cast<int16_t>(b.get(10));
	h.solar_s1_current = static_cast<int16_t>(b.get(10));
	h.solar_s2_current = static_cast<int16_t>(b.get(10));
	h.solar_w1_current = static_cast<int16_t>(b.get(10));
	h.solar_w2_current = static_cast<int16_t>(b.get(10));
	h.solar_t1_current = static_cast<int16_t>(b.get(10));
	h.solar_t2_current = static_cast<int16_t>(b.get(10));
	bool rails_switch_status[6];
	for (int i=0; i<6; i++) { rails_switch_status[i] = static_cast<bool>(b.get(1)); }
	std::copy(rails_switch_status, rails_switch_status+6, h.rails_switch_status);
	bool rails_overcurrent_status[6];
	for (int i=0; i<6; i++) { rails_overcurrent_status[i] = static_cast<bool>(b.get(1)); }
	...
	h.rail_1_voltage = static_cast<int16_t>(b.get(10));
	h.rail_1_current = static_cast<int16_t>(b.get(10));
	h.rail_2_boot_count = static_cast<uint16_t>(b.get(16));
	h.rail_2_overcurrent_count = static_cast<uint16_t>(b.get(16));
	h.rail_2_voltage = static_cast<int16_t>(b.get(10));
	h.rail_2_current = static_cast<int16_t>(b.get(10));
	h.rail_3_boot_count = static_cast<uint16_t>(b.get(16));
	h.rail_3_overcurrent_count = static_cast<uint16_t>(b.get(16));
	h.rail_3_voltage = static_cast<int16_t>(b.get(10));
	h.rail_3_current = static_cast<int16_t>(b.get(10));
	h.rail_4_boot_count = static_cast<uint16_t>(b.get(16));
	h.rail_4_overcurrent_count = static_cast<uint16_t>(b.get(16));
	h.rail_4_voltage = static_cast<int16_t>(b.get(10));
	h.rail_4_current = static_cast<int16_t>(b.get(10));
	h.rail_5_boot_count = static_cast<uint16_t>(b.get(16));
	h.rail_5_overcurrent_count = static_cast<uint16_t>(b.get(16));
	h.rail_5_voltage = static_cast<int16_t>(b.get(10));
	h.rail_5_current = static_cast<int16_t>(b.get(10));
	h.rail_6_boot_count = static_cast<uint16_t>(b.get(16));
	h.rail_6_overcurrent_count = static_cast<uint16_t>(b.get(16));
	h.rail_6_voltage = static_cast<int16_t>(b.get(10));
	h.rail_6_current = static_cast<int16_t>(b.get(10));
	h._3v3_voltage = static_cast<int16_t>(b.get(10));
	h._3v3_current = static_cast<int16_t>(b.get(10));
	h._5v_voltage = static_cast<int16_t>(b.get(10));
	h._5v_current = static_cast<int16_t>(b.get(10));
...
void from_buffer (Status& s, Buffer& b) {
	...
	for (int i=0; i<2; i++) { spacecraft_id[i] = static_cast<char>(b.get(8)); }
	spacecraft_id[2] = '\0';
	std::copy(spacecraft_id, spacecraft_id+3, s.spacecraft_id);
	s.time = static_cast<uint32_t>(b.get(32));
	s.time_source = static_cast<bool>(b.get(1));
	char beacon_id[5];
	for (int i=0; i<4; i++) { beacon_id[i] = static_cast<char>(b.get(8)); }
	beacon_id[4] = '\0';
	std::copy(beacon_id, beacon_id+5, s.beacon_id);
	s.obc_temperature = static_cast<int8_t>(b.get(8));
	s.battery_temperature = static_cast<int8_t>(b.get(8));
	s.battery_voltage = static_cast<int8_t>(b.get(8));
	s.battery_current = static_cast<int8_t>(b.get(8));
	s.charge_current = static_cast<int8_t>(b.get(8));
	s.antenna_deployment = static_cast<bool>(b.get(1));
	s.operational_mode = static_cast<uint8_t>(b.get(2));
	s.data_pending = static_cast<uint16_t>(b.get(16));
	s.reboot_count = static_cast<uint8_t>(b.get(8));
	bool rails_status[6];
	for (int i=0; i<6; i++) { rails_status[i] = static_cast<bool>(b.get(1)); }
...
void from_buffer (Packet& p, Buffer& b, uint32_t downlink_time) {
	...
	strncpy(p.hash, b.get_buf()+b.get_len()-(16+2)*8, 16);
